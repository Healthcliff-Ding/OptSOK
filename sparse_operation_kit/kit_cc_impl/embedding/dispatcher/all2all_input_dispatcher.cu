...
class All2AllInputDispatcher : public Dispatcher {
 public:
  ...
  void forward(const Context_t &replica_context, const bool training) override {
    ...
    // start Profiling
    gettimeofday(&begin[local_replica_id], 0);
    ...
    gettimeofday(&end[local_replica_id], 0);
    cpu_time = (1000000.0 * (end[local_replica_id].tv_sec - begin[local_replica_id].tv_sec) + 
                end[local_replica_id].tv_usec - begin[local_replica_id].tv_usec)/1000.0;
