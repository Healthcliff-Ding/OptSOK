...
def _IndexedSlicesToTensorDisableWarning(indexed_slices):
    ...
    if indexed_slices.dense_shape is None:
        raise ValueError(
            "Convert IndexedSlices to Tensor needs dense_shape: %s" % (str(indexed_slices))
        )
    ...
    return sok_ops.unsorted_segment_sum(
        indexed_slices.values, indexed_slices.indices, indexed_slices.dense_shape[0]
    )
...
def _PluginSparseBackProp(op, top_grad, nnz_grad):
    ...
    grads = ops.IndexedSlices(
        values=emb_var_grads_value, indices=value_index, dense_shape=params_shape
    )
