...
if __name__ == "__main__":
    ...
    parser.add_argument("--slot_num", type=int, help="the number of feature fields", required=True)
    parser.add_argument(
        "--nnz_per_slot", type=int, help="the number of keys in each slot", required=True
    )
    parser.add_argument("--vocabulary_size", type=int, required=False, default=1024 * 8)
    parser.add_argument(
        "--iter_num", type=int, help="the number of training iterations", required=True
    )
    parser.add_argument(
        "--filename",
        type=str,
